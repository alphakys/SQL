...
from locations lo, departments de, (select department_id 
                                                from employees 
                                                where first_name = 'Steven' 
                                                and last_name = 'King') SteveDe
                                
                                
where SteveDe.department_id = de.department_id and de.location_id = lo.location_id;
...
where e.department_id = maxSal.department_id and e.salary = maxSal.salary;
...
from jobs j,(select job_id, sum(salary) salary
             from employees
             group by job_id) sumSal

where j.job_id = sumSal.job_id;


select job_id, salary

from employees
where (job_id, salary) in (select job_id, sum(salary)
                            from employees
                            group by job_id);
...
from  (select       employee_id,
                    first_name,
                     hire_date,
                     salary,
                    rank() over (order by hire_date asc) rn1
                    from employees) r

where r.rn1 between 11 and 15;
...
select employee_id,
        ...
        dense_rank() over (order by hire_date asc) dense_rank
from employees
